...
const UserRecoverForm = () => {
    ...
    const sendEmail = async (e) => {
        e.preventDefault()
        try {
            const res = await axios.post('/api/user/recover', { email: formData.email }, { withCredentials: true })
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        try {
            const res = await axios.post('/api/user/recover/reset', formData, { withCredentials: true })
            toast.success(res.data.message)
        ...
        <div className='w-full flex flex-col items-center gap-4' >
            {
                !otpSent ? (
                    ...
                    <form onSubmit={handleSubmit} className='w-full flex flex-col items-center gap-4'>
                        ...
                        <Link href={'/user-login'} className='w-full text-right'>Login</Link>
...
export default UserRecoverForm